    ...
    <div class="main-split-layout">
            ...
            >
              ...
              <div class="section-body" v-show="!collapsedSections.has(idx)">
                ...
                <div v-else-if="currentSectionIndex === idx + 1" class="loading-state">
                  ...
                  <span class="loading-text">Generando {{ section.title }}...</span>
                </div>
              </div>
            </div>
          </div>
      ...
      <div class="right-panel" ref="rightPanel">
        ...
        <div class="workflow-overview" v-if="agentLogs.length > 0 || reportOutline">
          ...
          <!-- Next Step Button - mostrar al completar -->
          <button v-if="isComplete" class="next-step-btn" @click="goToInteraction">
            <span>Ir a Interacción profunda</span>
          ...
          <button v-if="isComplete" class="download-report-btn" @click="downloadReportTxt">
            ...
            >
              ...
              <div class="timeline-content">
                ...
                <div class="timeline-body" :class="{ 'collapsed': isLogCollapsed(log) }" @click="toggleLogExpand(log)">
                  ...
                  <!-- Section Content Generated (contenido generado, pero la sección puede no estar completada) -->
                  ...
                  <!-- Section Complete (generación de sección completada) -->
                  ...
                  <template v-if="log.action === 'llm_response'">
                    ...
                    <!-- Mostrar aviso especial cuando es la respuesta final -->
...
const downloadReportTxt = () => {
  if (props.reportId) {
    const url = `/api/report/${props.reportId}/download?format=txt`
    const a = document.createElement('a')
    a.href = url
    a.download = `${props.reportId}.txt`
...
const toggleRawResult = (timestamp, event) => {
  // Guardar la posición del botón relativa al viewport
  ...
  // Esperar actualización del DOM y ajustar la posición de scroll para mantener el botón en el mismo lugar
...
const toolConfig = {
  'insight_forge': {
    name: 'Deep Insight',
    color: 'purple',
    icon: 'lightbulb' // Ícono de bombilla - representa insights
  },
  'panorama_search': {
  ...
  'interview_agents': {
    ...
    color: 'green',
    icon: 'users' // Ícono de usuarios - representa conversación
  },
  'quick_search': {
    name: 'Quick Search',
    color: 'orange',
    icon: 'zap' // Ícono de rayo - representa rapidez
  },
  ...
  'get_entities_by_type': {
    name: 'Entity Query',
    color: 'pink',
    icon: 'database' // Ícono de base de datos - representa entidades
  }
}
...
const parseInsightForge = (text) => {
  ...
  try {
    // Extraer pregunta de análisis
    const queryMatch = text.match(/分析问题:\s*(.+?)(?:\n|$)/)
    if (queryMatch) result.query = queryMatch[1].trim()
    
    // Extraer escenario de predicción
    const reqMatch = text.match(/预测场景:\s*(.+?)(?:\n|$)/)
    if (reqMatch) result.simulationRequirement = reqMatch[1].trim()
    
    // Extraer estadísticas - coincide con formato del backend "相关预测事实: X条"
    ...
    // Extraer sub-preguntas - extracción completa, sin límite de cantidad
    ...
    // Extraer hechos clave - extracción completa, sin límite de cantidad
    ...
    // Extraer entidades clave - extracción completa, incluyendo resumen y conteo de hechos relacionados
    const entitySection = text.match(/### 【核心实体】\n([\s\S]*?)(?=\n###|$)/)
    if (entitySection) {
      const entityText = entitySection[1]
      // Dividir bloques de entidades por "- **"
    ...
    // Extraer cadenas de relaciones - extracción completa, sin límite de cantidad
...
const parsePanorama = (text) => {
  ...
  try {
    ...
    // Extraer hechos válidos actuales - extracción completa, sin límite de cantidad
    const activeSection = text.match(/### 【当前有效事实】[\s\S]*?\n([\s\S]*?)(?=\n###|$)/)
    if (activeSection) {
      const lines = activeSection[1].split('\n').filter(l => l.match(/^\d+\./))
      result.activeFacts = lines.map(l => {
        // Eliminar numeración y comillas
        const factText = l.replace(/^\d+\.\s*/, '').replace(/^"|"$/g, '').trim()
        return factText
      }).filter(Boolean)
    }
    
    // Extraer hechos históricos/vencidos - extracción completa, sin límite de cantidad
    ...
    // Extraer entidades involucradas - extracción completa, sin límite de cantidad
...
const parseInterview = (text) => {
  ...
  try {
    // Extraer tema de la entrevista
    const topicMatch = text.match(/\*\*采访主题:\*\*\s*(.+?)(?:\n|$)/)
    if (topicMatch) result.topic = topicMatch[1].trim()
    
    // Extraer cantidad de entrevistados (ej. "5 / 9 Agentes de simulación")
    ...
    // Extraer motivo de selección de entrevistados
    const reasonMatch = text.match(/### 采访对象选择理由\n([\s\S]*?)(?=\n---\n|\n### 采访实录)/)
    if (reasonMatch) {
      result.selectionReason = reasonMatch[1].trim()
    }
    
    // Analizar el motivo de selección de cada persona
    const parseIndividualReasons = (reasonText) => {
      ...
      for (const line of lines) {
        ...
        // Formato 1: Número. **Nombre（index=X）**: Motivo
        // Ej: 1. **Agente_345（index=1）**: Como exalumno...
        ...
        // Formato 2: - Seleccionar Nombre（index X）: Motivo
        // Ej: - Seleccionar Agente_601（index 0）: Como representante del grupo de padres...
        if (!headerMatch) {
        ...
        // Formato 3: - **Nombre（index X）**: Motivo
        // Ej: - **Agente_601（index 0）**: Como representante del grupo de padres...
        if (!headerMatch) {
        ...
        if (name) {
          // Guardar el motivo de la persona anterior
          if (currentName && currentReason.length > 0) {
            reasons[currentName] = currentReason.join(' ').trim()
          }
          // Comenzar nueva persona
          currentName = name
          currentReason = reasonStart ? [reasonStart.trim()] : []
        } else if (currentName && line.trim() && !line.match(/^未选|^综上|^最终选择/)) {
          // Línea de continuación del motivo (excluyendo párrafos de resumen final)
          currentReason.push(line.trim())
        }
      }
      
      // Guardar el motivo de la última persona
      if (currentName && currentReason.length > 0) {
        reasons[currentName] = currentReason.join(' ').trim()
      }
    ...
    // Extraer cada registro de entrevista
    ...
    interviewBlocks.forEach((block, index) => {
      ...
      // Extraer título (ej. "Estudiante", "Profesional de la educación", etc.)
      ...
      if (nameRoleMatch) {
        ...
        // Establecer el motivo de selección de esta persona
      ...
      if (qMatch) {
        ...
        // Dividir preguntas por numeración
        const questions = qText.split(/\n\d+\.\s+/).filter(q => q.trim())
        if (questions.length > 0) {
          // Si la primera pregunta tiene "1." al inicio, necesita manejo especial
      ...
      // Extraer respuesta - separar Twitter y Reddit
      const answerMatch = block.match(/\*\*A:\*\*\s*([\s\S]*?)(?=\*\*关键引言|$)/)
      if (answerMatch) {
        const answerText = answerMatch[1].trim()
        
        // Separar respuestas de Twitter y Reddit
        ...
        // Lógica de fallback de plataforma (compatibilidad con formato antiguo: solo un marcador de plataforma)
        if (!twitterMatch && redditMatch) {
          // Solo respuesta Reddit, copiar como visualización predeterminada solo si no es texto placeholder
          ...
          // Sin marcador de plataforma (formato muy antiguo), usar como respuesta completa
          interview.twitterAnswer = answerText
        }
      }
      
      // Extraer citas clave (compatible con múltiples formatos de comillas)
      const quotesMatch = block.match(/\*\*关键引言:\*\*\n([\s\S]*?)(?=\n---|\n####|$)/)
      if (quotesMatch) {
        const quotesText = quotesMatch[1]
        // Priorizar coincidencia con formato > "text"
        let quoteMatches = quotesText.match(/> "([^"]+)"/g)
        // Fallback: coincidir con > "text" o > \u201Ctext\u201D (comillas chinas)
        if (!quoteMatches) {
    ...
    // Extraer resumen de la entrevista
...
const parseQuickSearch = (text) => {
  ...
  try {
    ...
    // Extraer hechos relacionados - extracción completa, sin límite de cantidad
    ...
    // Intentar extraer información de aristas (si existe)
    ...
    // Intentar extraer información de nodos (si existe)
    ...
    return () => h('div', { class: 'insight-display' }, [
      ...
      h('div', { class: 'insight-header' }, [
        ...
        props.result.simulationRequirement && h('div', { class: 'header-scenario' }, [
          h('span', { class: 'scenario-label' }, 'Escenario de predicción: '),
      ...
      h('div', { class: 'insight-tabs' }, [
        h('button', {
          ...
          h('span', { class: 'tab-label' }, `Memoria clave actual (${props.result.facts.length})`)
        ]),
        h('button', {
          class: ['insight-tab', { active: activeTab.value === 'entities' }],
          onClick: () => { activeTab.value = 'entities' }
        }, [
          h('span', { class: 'tab-label' }, `Entidades clave (${props.result.entities.length})`)
        ]),
        h('button', {
          class: ['insight-tab', { active: activeTab.value === 'relations' }],
          onClick: () => { activeTab.value = 'relations' }
        }, [
          h('span', { class: 'tab-label' }, `Cadenas de relación (${props.result.relations.length})`)
        ]),
        props.result.subQueries.length > 0 && h('button', {
          ...
          h('span', { class: 'tab-label' }, `Sub-preguntas (${props.result.subQueries.length})`)
        ])
      ]),
      ...
      h('div', { class: 'insight-content' }, [
        ...
        activeTab.value === 'facts' && props.result.facts.length > 0 && h('div', { class: 'facts-panel' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, 'Últimos hechos clave asociados en la memoria temporal'),
          ...
          }, expandedFacts.value ? `Colapsar ▲` : `Expandir todo (${props.result.facts.length}) ▼`)
        ]),
        
        // Entities Tab
        activeTab.value === 'entities' && props.result.entities.length > 0 && h('div', { class: 'entities-panel' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, 'Entidades clave'),
          ...
          }, expandedEntities.value ? `Colapsar ▲` : `Expandir todo (${props.result.entities.length}) ▼`)
        ]),
        
        // Relations Tab
        activeTab.value === 'relations' && props.result.relations.length > 0 && h('div', { class: 'relations-panel' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, 'Cadenas de relación'),
          ...
          }, expandedRelations.value ? `Colapsar ▲` : `Expandir todo (${props.result.relations.length}) ▼`)
        ]),
        
        // Sub-queries Tab
        activeTab.value === 'subqueries' && props.result.subQueries.length > 0 && h('div', { class: 'subqueries-panel' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, 'Análisis de sub-preguntas generadas por consulta de deriva'),
        ...
        // Empty state
        activeTab.value === 'facts' && props.result.facts.length === 0 && h('div', { class: 'empty-state' }, 'Sin hechos clave actuales'),
        activeTab.value === 'entities' && props.result.entities.length === 0 && h('div', { class: 'empty-state' }, 'Sin entidades clave'),
        activeTab.value === 'relations' && props.result.relations.length === 0 && h('div', { class: 'empty-state' }, 'Sin cadenas de relación')
      ])
    ])
  }
}
    ...
    return () => h('div', { class: 'panorama-display' }, [
      ...
      h('div', { class: 'panorama-tabs' }, [
        h('button', {
          ...
          h('span', { class: 'tab-label' }, `Memoria válida actual (${props.result.activeFacts.length})`)
        ]),
        h('button', {
          class: ['panorama-tab', { active: activeTab.value === 'historical' }],
          onClick: () => { activeTab.value = 'historical' }
        }, [
          h('span', { class: 'tab-label' }, `Memoria histórica (${props.result.historicalFacts.length})`)
        ]),
        h('button', {
          class: ['panorama-tab', { active: activeTab.value === 'entities' }],
          onClick: () => { activeTab.value = 'entities' }
        }, [
          h('span', { class: 'tab-label' }, `Entidades involucradas (${props.result.entities.length})`)
        ])
      ]),
      ...
      h('div', { class: 'panorama-content' }, [
        ...
        activeTab.value === 'active' && h('div', { class: 'facts-panel active-facts' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, 'Memoria válida actual'),
        ...
        activeTab.value === 'historical' && h('div', { class: 'facts-panel historical-facts' }, [
          ...
          ) : h('div', { class: 'empty-state' }, 'Sin memoria histórica'),
          props.result.historicalFacts.length > INITIAL_SHOW_COUNT && h('button', {
            class: 'expand-btn',
            onClick: () => { expandedHistorical.value = !expandedHistorical.value }
          }, expandedHistorical.value ? `Colapsar ▲` : `Expandir todo (${props.result.historicalFacts.length}) ▼`)
        ]),
        
        // Entities Tab
        activeTab.value === 'entities' && h('div', { class: 'entities-panel' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, 'Entidades involucradas'),
            h('span', { class: 'panel-count' }, `Total: ${props.result.entities.length}`)
          ]),
          ...
          ) : h('div', { class: 'empty-state' }, 'Sin entidades involucradas'),
...
const InterviewDisplay = {
  ...
  setup(props) {
    ...
    // Mantener estado de selección de plataforma independiente para cada par pregunta-respuesta
    const platformTabs = reactive({}) // { 'agentIdx-qIdx': 'twitter' | 'reddit' }
    
    // Obtener la selección de plataforma actual para una pregunta
    const getPlatformTab = (agentIdx, qIdx) => {
      const key = `${agentIdx}-${qIdx}`
      return platformTabs[key] || 'twitter'
    }
    
    // Establecer la selección de plataforma para una pregunta
    ...
    // Verificar si es texto placeholder de plataforma
    ...
    // Intentar dividir respuesta por número de pregunta
    const splitAnswerByQuestions = (answerText, questionCount) => {
      ...
      // Soportar dos formatos de numeración:
      // 1. "问题X：" o "问题X:" (formato chino del backend, nuevo formato)
      // 2. "1. " o "\n1. " (número+punto, compatibilidad con formato antiguo)
      let matches = []
      let match

      // Intentar primero el formato chino "问题X："
      const cnPattern = /(?:^|[\r\n]+)问题(\d+)[：:]\s*/g
      ...
      // Si no se encontró numeración o solo se encontró una, retornar el conjunto
      if (matches.length <= 1) {
      ...
      // Extraer cada parte por numeración
      const parts = []
    ...
    // Obtener la respuesta correspondiente a una pregunta
    const getAnswerForQuestion = (interview, qIdx, platform) => {
      ...
      // División exitosa y el índice es válido
      if (answers.length > 1 && qIdx < answers.length) {
        return answers[qIdx] || ''
      }

      // División fallida: la primera pregunta retorna la respuesta completa, las demás retornan vacío
      return qIdx === 0 ? answer : ''
    }
    
    // Verificar si una pregunta tiene respuesta de dos plataformas (filtrar texto placeholder)
    const hasMultiplePlatforms = (interview, qIdx) => {
      ...
      // Ambas plataformas tienen respuesta real (no placeholder) y el contenido es diferente
              ...
              answerText && h('div', { class: ['qa-answer', { 'answer-placeholder': isPlaceholder }] }, [
                ...
                h('div', { class: 'qa-content' }, [
                  h('div', { class: 'qa-answer-header' }, [
                    ...
                    // Botón de cambio entre plataformas (mostrar solo cuando hay respuesta real de dos plataformas)
                  ...
                  // Expand/Collapse Button (no mostrar texto placeholder)
    ...
    return () => h('div', { class: 'quick-search-display' }, [
      ...
      h('div', { class: 'quicksearch-header' }, [
        ...
        props.result.query && h('div', { class: 'header-query' }, [
          h('span', { class: 'query-label' }, 'Búsqueda: '),
      ...
      h('div', { class: ['quicksearch-content', { 'no-tabs': !showTabs.value }] }, [
        ...
        ((!showTabs.value) || activeTab.value === 'facts') && h('div', { class: 'facts-panel' }, [
          !showTabs.value && h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, 'Resultados de búsqueda'),
            h('span', { class: 'panel-count' }, `Total: ${props.result.facts.length}`)
          ]),
          ...
          ) : h('div', { class: 'empty-state' }, 'No se encontraron resultados relacionados'),
          props.result.facts.length > INITIAL_SHOW_COUNT && h('button', {
            class: 'expand-btn',
            onClick: () => { expandedFacts.value = !expandedFacts.value }
          }, expandedFacts.value ? `Colapsar ▲` : `Expandir todo (${props.result.facts.length}) ▼`)
        ...
        activeTab.value === 'edges' && hasEdges.value && h('div', { class: 'edges-panel' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, 'Relaciones relacionadas'),
        ...
        activeTab.value === 'nodes' && hasNodes.value && h('div', { class: 'nodes-panel' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, 'Nodos relacionados'),
...
// Paso activo actual (para mostrar en la parte superior)
const activeStep = computed(() => {
  ...
  // Si no hay activo, retornar el último paso "done"
  const doneSteps = steps.filter(s => s.status === 'done')
  if (doneSteps.length > 0) return doneSteps[doneSteps.length - 1]
  
  // De lo contrario retornar el primer paso
  return steps[0] || { noLabel: '--', title: 'Esperando inicio', status: 'todo', meta: '' }
})
...
const renderMarkdown = (content) => {
  ...
  // Eliminar el título de nivel 2 inicial (## xxx) porque el título de sección ya se muestra afuera
  ...
  // Procesar bloques de cita
  html = html.replace(/^> (.+)$/gm, '<blockquote class="md-quote">$1</blockquote>')
  
  // Procesar listas - soportar sub-listas
  ...
  // Envolver lista desordenada
  html = html.replace(/(<li class="md-li"[^>]*>.*?<\/li>\s*)+/g, '<ul class="md-ul">$&</ul>')
  // Envolver lista ordenada
  html = html.replace(/(<li class="md-oli"[^>]*>.*?<\/li>\s*)+/g, '<ol class="md-ol">$&</ol>')

  // Limpiar todos los espacios en blanco entre ítems de lista
  html = html.replace(/<\/li>\s+<li/g, '</li><li')
  // Limpiar espacios en blanco después de la etiqueta de inicio de lista
  html = html.replace(/<ul class="md-ul">\s+/g, '<ul class="md-ul">')
  html = html.replace(/<ol class="md-ol">\s+/g, '<ol class="md-ol">')
  // Limpiar espacios en blanco antes de la etiqueta de fin de lista
  ...
  // Procesar líneas divisorias
  html = html.replace(/^---$/gm, '<hr class="md-hr">')
  
  // Procesar saltos de línea - línea vacía se convierte en separador de párrafo, salto simple en <br>
  html = html.replace(/\n\n/g, '</p><p class="md-p">')
  html = html.replace(/\n/g, '<br>')
  ...
  // Limpiar el caso de <p><br> seguido de elemento de bloque (causado por líneas vacías extras)
  html = html.replace(/<p class="md-p">(<br>\s*)+(<ul|<ol|<blockquote|<pre|<hr)/g, '$2')
  // Limpiar etiquetas <br> consecutivas
  html = html.replace(/(<br>\s*){2,}/g, '<br>')
  // Limpiar el <br> antes de la etiqueta de inicio de párrafo que sigue a un elemento de bloque
  html = html.replace(/(<\/ol>|<\/ul>|<\/blockquote>)<br>(<p|<div)/g, '$1$2')

  // Corregir numeración de listas ordenadas no continuas: mantener incremento cuando <ol> individual está separado por párrafos
...
const getLogLevelClass = (log) => {
  ...
  // INFO usa el color predeterminado, no marcar como success
  return ''
}
    ...
    if (res.success && res.data) {
      ...
      if (newLogs.length > 0) {
        newLogs.forEach(log => {
          ...
          // section_complete - generación de sección completada
          if (log.action === 'section_complete') {
            if (log.details?.content) {
              generatedSections.value[log.section_index] = log.details.content
              // Expandir automáticamente la sección recién generada
          ...
          if (log.action === 'report_complete') {
            ...
            currentSectionIndex.value = null  // Asegurar que el estado de loading se limpia
            emit('update-status', 'completed')
            stopPolling()
            // La lógica de scroll se maneja en nextTick después de que el ciclo termine
          }
        ...
        nextTick(() => {
          if (rightPanel.value) {
            // Si la tarea está completada, ir al inicio; de lo contrario ir al final para seguir los últimos logs
            if (isComplete.value) {
...
// Extraer el contenido de la respuesta final - extraer contenido de sección de la respuesta del LLM
const extractFinalContent = (response) => {
  if (!response) return null
  
  // Intentar extraer el contenido dentro de la etiqueta <final_answer>
  ...
  // Intentar encontrar el contenido después de "Final Answer:" (soporta múltiples formatos)
  // Formato 1: Final Answer:\n\ncontenido
  // Formato 2: Final Answer: contenido
  const finalAnswerMatch = response.match(/Final\s*Answer:\s*\n*([\s\S]*)$/i)
  if (finalAnswerMatch) {
    return finalAnswerMatch[1].trim()
  }
  
  // Intentar encontrar el contenido después de "最终答案:" (clave de respuesta final del backend)
  const chineseFinalMatch = response.match(/最终答案[:：]\s*\n*([\s\S]*)$/i)
  if (chineseFinalMatch) {
    return chineseFinalMatch[1].trim()
  }
  
  // Si comienza con ## o # o >, puede ser contenido markdown directo
  const trimmedResponse = response.trim()
  if (trimmedResponse.match(/^[#>]/)) {
    return trimmedResponse
  }
  
  // Si el contenido es largo y contiene formato markdown, intentar eliminar el proceso de pensamiento antes de retornar
  if (response.length > 300 && (response.includes('**') || response.includes('>'))) {
    // Eliminar el proceso de pensamiento que comienza con "Thought:"
...
.section-number {
  ...
  font-size: 16px;
  color: #9CA3AF; /* Gris oscuro, no cambia con el estado */
  font-weight: 500;
}
...
/* Console Logs - mantener consistente con Step3Simulation.vue */
.console-logs {
  background: #000;
  color: #DDD;
  padding: 16px;
