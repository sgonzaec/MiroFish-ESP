        ...
        <div class="action-bar-header">
          ...
          <div class="action-bar-tabs">
            ...
            >
              ...
              <span>Hablar con Report Agent</span>
            </button>
            <div class="agent-dropdown" v-if="profiles.length > 0">
              ...
              >
                ...
                <span>{{ selectedAgent ? selectedAgent.username : 'Hablar con cualquier individuo del mundo' }}</span>
                <svg class="dropdown-arrow" :class="{ open: showAgentDropdown }" viewBox="0 0 24 24" width="12" height="12" fill="none" stroke="currentColor" stroke-width="2">
                  <polyline points="6 9 12 15 18 9"></polyline>
                </svg>
              </button>
              <div v-if="showAgentDropdown" class="dropdown-menu">
                <div class="dropdown-header">Seleccionar interlocutor</div>
                <div 
                ...
                >
                  ...
                  <div class="agent-info">
                    ...
                    <span class="agent-role">{{ agent.profession || 'Profesión desconocida' }}</span>
                  </div>
                </div>
              </div>
            </div>
            ...
            >
              ...
              <span>Enviar encuesta al mundo</span>
            </button>
          </div>
        </div>
        ...
        <div v-if="activeTab === 'chat'" class="chat-container">
          ...
          <div v-if="chatTarget === 'report_agent'" class="report-agent-tools-card">
            <div class="tools-card-header">
              ...
              <div class="tools-card-info">
                ...
                <div class="tools-card-subtitle">Versión de chat rápido del agente generador de informes, con acceso a 4 herramientas especializadas y memoria completa de MiroFish</div>
              </div>
              ...
              <div class="tools-grid">
                <div class="tool-item tool-purple">
                  ...
                  <div class="tool-content">
                    <div class="tool-name">InsightForge — Atribución profunda</div>
                    <div class="tool-desc">Alinea datos semilla del mundo real con el estado del entorno simulado. Combina mecanismos de memoria global/local para análisis de atribución profunda a través del tiempo</div>
                  </div>
                </div>
                <div class="tool-item tool-blue">
                  ...
                  <div class="tool-content">
                    <div class="tool-name">PanoramaSearch — Rastreo panorámico</div>
                    <div class="tool-desc">Algoritmo de recorrido en anchura sobre estructura de grafo. Reconstruye rutas de propagación de eventos y captura la topología completa del flujo de información</div>
                  </div>
                </div>
                <div class="tool-item tool-orange">
                  ...
                  <div class="tool-content">
                    <div class="tool-name">QuickSearch — Búsqueda rápida</div>
                    <div class="tool-desc">Interfaz de consulta instantánea basada en GraphRAG. Optimiza la eficiencia de indexación para extraer rápidamente atributos de nodos y hechos discretos</div>
                  </div>
                </div>
                <div class="tool-item tool-green">
                  ...
                  <div class="tool-content">
                    <div class="tool-name">InterviewSubAgent — Entrevista virtual</div>
                    <div class="tool-desc">Entrevista autónoma capaz de mantener conversaciones multi-ronda en paralelo con individuos del mundo simulado, recopilando datos de opinión y estados psicológicos</div>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div v-if="chatTarget === 'agent' && selectedAgent" class="agent-profile-card">
            <div class="profile-card-header">
              ...
              <div class="profile-card-info">
                ...
                <div class="profile-card-meta">
                  ...
                  <span class="profile-card-profession">{{ selectedAgent.profession || 'Profesión desconocida' }}</span>
                </div>
              </div>
            ...
            <div v-if="showFullProfile && selectedAgent.bio" class="profile-card-body">
              <div class="profile-card-bio">
                <div class="profile-card-label">Perfil</div>
          ...
          <div class="chat-messages" ref="chatMessages">
            <div v-if="chatHistory.length === 0" class="chat-empty">
              ...
              <p class="empty-text">
                {{ chatTarget === 'report_agent' ? 'Conversa con Report Agent para explorar el informe en profundidad' : 'Conversa con individuos simulados para conocer sus perspectivas' }}
              </p>
            </div>
            <div 
        ...
        <div v-if="activeTab === 'survey'" class="survey-container">
          ...
          <div class="survey-setup">
            <div class="setup-section">
              <div class="section-header">
                <span class="section-title">Seleccionar destinatarios</span>
                <span class="selection-count">Seleccionados: {{ selectedAgents.size }} / {{ profiles.length }}</span>
              </div>
              <div class="agents-grid">
                ...
                >
                  ...
                  <div class="checkbox-info">
                    ...
                    <span class="checkbox-role">{{ agent.profession || 'Profesión desconocida' }}</span>
                  </div>
              ...
              <div class="selection-actions">
                <button class="action-link" @click="selectAllAgents">Seleccionar todo</button>
                <span class="action-divider">|</span>
                <button class="action-link" @click="clearAgentSelection">Limpiar</button>
              </div>
            </div>

            <div class="setup-section">
              <div class="section-header">
                <span class="section-title">Preguntas de la encuesta</span>
              </div>
              <textarea 
                v-model="surveyQuestion"
                class="survey-input"
                placeholder="Escribe la pregunta para todos los seleccionados..."
                rows="3"
              ></textarea>
            </div>
            ...
            >
              ...
              <span v-else>Enviar encuesta</span>
            </button>
          </div>
          ...
          <div v-if="surveyResults.length > 0" class="survey-results">
            <div class="results-header">
              <span class="results-title">Resultados de la encuesta</span>
              <div class="results-header-right">
                <span class="results-count">{{ surveyResults.length }} respuestas</span>
                <button class="download-survey-btn" @click="downloadSurveyResults" title="Descargar resultados como TXT">
              ...
              >
                <div class="result-header">
                  ...
                  <div class="result-info">
                    ...
                    <span class="result-role">{{ result.profession || 'Profesión desconocida' }}</span>
                  </div>
                </div>
...
const chatHistoryCache = ref({}) // Caché de todos los historiales de conversación: { 'report_agent': [], 'agent_0': [], ... }
...
// Guardar historial de conversación actual en caché
...
const selectReportAgentChat = () => {
  // Guardar historial de conversación actual
  saveChatHistory()
  ...
  // Restaurar historial de conversación de Report Agent
...
const selectAgent = (agent, idx) => {
  ...
  // Restaurar historial de conversación de este agente
  chatHistory.value = chatHistoryCache.value[`agent_${idx}`] || []
  addLog(`Interlocutor seleccionado: ${agent.username}`)
}
...
const renderMarkdown = (content) => {
  ...
  // Procesar listas - soporta sublistas
  ...
  // Limpiar espacios entre ítems de lista
  html = html.replace(/<\/li>\s+<li/g, '</li><li')
  // Limpiar espacios después de etiqueta de apertura de lista
  html = html.replace(/<ul class="md-ul">\s+/g, '<ul class="md-ul">')
  html = html.replace(/<ol class="md-ol">\s+/g, '<ol class="md-ol">')
  // Limpiar espacios antes de etiqueta de cierre de lista
  ...
  // Limpiar etiquetas <br> antes/después de elementos de bloque
  html = html.replace(/<br>\s*(<ul|<ol|<blockquote)/g, '$1')
  html = html.replace(/(<\/ul>|<\/ol>|<\/blockquote>)\s*<br>/g, '$1')
  // Limpiar <p><br> seguido de elementos de bloque (líneas vacías redundantes)
  html = html.replace(/<p class="md-p">(<br>\s*)+(<ul|<ol|<blockquote|<pre|<hr)/g, '$2')
  // Limpiar etiquetas <br> consecutivas
  html = html.replace(/(<br>\s*){2,}/g, '<br>')
  // Limpiar <br> antes de etiqueta de párrafo que sigue a un bloque
  html = html.replace(/(<\/ol>|<\/ul>|<\/blockquote>)<br>(<p|<div)/g, '$1$2')

  // Corregir numeración de listas ordenadas no continuas: mantener numeración incremental cuando <ol> está separado por párrafos
...
const sendMessage = async () => {
  ...
  try {
    ...
    scrollToBottom()
    // Guardar historial de conversación automáticamente en caché
    saveChatHistory()
  }
}

const sendToReportAgent = async (message) => {
  addLog(`Enviando a Report Agent: ${message.substring(0, 50)}...`)
  ...
  if (res.success && res.data) {
    chatHistory.value.push({
      role: 'assistant',
      content: res.data.response || res.data.answer || 'Sin respuesta',
      timestamp: new Date().toISOString()
    })
    addLog('Report Agent ha respondido')
  } else {
    throw new Error(res.error || 'Solicitud fallida')
  }
}

const sendToAgent = async (message) => {
  if (!selectedAgent.value || selectedAgentIndex.value === null) {
    throw new Error('Por favor selecciona un individuo simulado primero')
  }
  
  addLog(`Enviando a ${selectedAgent.value.username}: ${message.substring(0, 50)}...`)
  ...
  if (chatHistory.value.length > 1) {
    const historyContext = chatHistory.value
      ...
      .join('\n')
    prompt = `Esta es nuestra conversación anterior:\n${historyContext}\n\nAhora mi nueva pregunta es: ${message}`
  }
  ...
  if (res.success && res.data) {
    // Ruta correcta de datos: res.data.result.results es un diccionario de objetos
    // Formato: {"twitter_0": {...}, "reddit_0": {...}} o plataforma única {"reddit_0": {...}}
    const resultData = res.data.result || res.data
    const resultsDict = resultData.results || resultData

    // Convertir diccionario de objetos a array, priorizar respuestas de plataforma reddit
    let responseContent = null
    const agentId = selectedAgentIndex.value
    
    if (typeof resultsDict === 'object' && !Array.isArray(resultsDict)) {
      // Priorizar respuesta de reddit, luego twitter
      ...
      // Compatible con formato de array
    ...
    if (responseContent) {
      ...
      addLog(`${selectedAgent.value.username} ha respondido`)
    } else {
...
const submitSurvey = async () => {
  ...
  addLog(`Enviando encuesta a ${selectedAgents.value.size} destinatarios...`)
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // Convertir diccionario de objetos a formato de array
      ...
      for (const interview of interviews) {
        ...
        if (typeof resultsDict === 'object' && !Array.isArray(resultsDict)) {
          ...
          if (agentResult) {
            responseContent = agentResult.response || agentResult.answer || 'Sin respuesta'
          }
        } else if (Array.isArray(resultsDict)) {
          // Compatible con formato de array
          const matchedResult = resultsDict.find(r => r.agent_id === agentIdx)
          if (matchedResult) {
            responseContent = matchedResult.response || matchedResult.answer || 'Sin respuesta'
          }
        }
      ...
      addLog(`Recibidas ${surveyResults.value.length} respuestas`)
    } else {
      throw new Error(res.error || 'Solicitud fallida')
    }
  } catch (err) {
    addLog(`Error al enviar encuesta: ${err.message}`)
  } finally {
    isSurveying.value = false
  }
}

const downloadSurveyResults = () => {
  ...
  let txt = `${sep}\n`
  txt += `  RESULTADOS DE ENCUESTA — MIROFISH\n`
  txt += `${sep}\n\n`
  txt += `Fecha: ${dateStr}  ${timeStr}\n`
  txt += `Total de respuestas: ${surveyResults.value.length}\n\n`
  txt += `${divider}\n`
  txt += `PREGUNTA DE LA ENCUESTA\n`
  txt += `${divider}\n`
  txt += `${surveyResults.value[0]?.question || ''}\n\n`
  txt += `${sep}\n\n`

  surveyResults.value.forEach((result, idx) => {
    txt += `RESPUESTA ${String(idx + 1).padStart(2, '0')}\n`
    txt += `${divider}\n`
    txt += `Agente:      ${result.agent_name || `Agent ${result.agent_id}`}\n`
    if (result.profession) {
      txt += `Descripción: ${result.profession}\n`
    }
    txt += `\n${result.answer || 'Sin respuesta'}\n\n`
  })

  txt += `${sep}\n`
  txt += `Generado con MiroFish en Español - EnPíxeles Lab\n`
  txt += `${sep}\n`
  ...
  a.href = url
  a.download = `encuesta_${now.toISOString().slice(0, 10)}_${now.getHours()}h${String(now.getMinutes()).padStart(2,'0')}.txt`
...
const loadReportData = async () => {
  ...
  try {
    addLog(`Cargando datos del informe: ${props.reportId}`)
    ...
    addLog(`Error al cargar informe: ${err.message}`)
  }
}
...
const loadAgentLogs = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      addLog('Datos del informe cargados correctamente')
    }
  } catch (err) {
    addLog(`Error al cargar logs del informe: ${err.message}`)
  }
}
...
const loadProfiles = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      addLog(`${profiles.value.length} individuos simulados cargados`)
    }
  } catch (err) {
    addLog(`Error al cargar individuos simulados: ${err.message}`)
  }
}
...
// Lifecycle
onMounted(() => {
  addLog('Step5: Interacción profunda inicializada')
  loadReportData()
  loadProfiles()
...
/* Left Panel - Report Style (idéntico a Step4Report.vue) */
.left-panel.report-style {
  width: 45%;
  min-width: 450px;
  background: #FFFFFF;
...
/* Corregir numeración de listas ordenadas - usar contadores CSS para numeración continua entre múltiples ol */
.message-text {
...
/* Estilos de cita para área de chat/encuesta */
