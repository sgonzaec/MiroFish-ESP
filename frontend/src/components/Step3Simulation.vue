  ...
  <div class="simulation-panel">
    ...
    <div class="control-bar">
      <div class="status-group">
        <!-- Progreso de la plataforma Twitter -->
        ...
        <!-- Progreso de la plataforma Reddit -->
      ...
      <div class="action-controls">
        ...
        >
          ...
          {{ isGeneratingReport ? 'Iniciando...' : 'Comenzar a generar informe de resultados' }} 
          ...
          >
            ...
            <div class="timeline-card">
              ...
              <div class="card-body">
                ...
                <!-- DO_NOTHING: Sin operación (silencio) -->
                ...
                <!-- Fallback genérico: tipo desconocido o con contenido no procesado arriba -->
...
const props = defineProps({
  simulationId: String,
  maxRounds: Number, // Máximo de rondas pasado desde Step2
  minutesPerRound: {
    type: Number,
    default: 30 // 30 minutos por ronda por defecto
  },
  projectData: Object,
  graphData: Object,
  systemLogs: Array
...
const runStatus = ref({})
const allActions = ref([]) // Todas las acciones (acumulación incremental)
const actionIds = ref(new Set()) // Conjunto de IDs de acciones para deduplicación
const scrollContainer = ref(null)

// Computed
// Mostrar acciones en orden cronológico (la más reciente al final, es decir, abajo)
...
// Formatear tiempo transcurrido de simulación (calculado por rondas y minutos por ronda)
...
// Tiempo de simulación transcurrido de la plataforma Twitter
const twitterElapsedTime = computed(() => {
  return formatElapsedTime(runStatus.value.twitter_current_round || 0)
})

// Tiempo de simulación transcurrido de la plataforma Reddit
...
// Reiniciar todos los estados (para reiniciar la simulación)
const resetAllState = () => {
  phase.value = 0
  runStatus.value = {}
  allActions.value = []
  ...
  stopPolling()  // Detener cualquier sondeo previo existente
}
...
const doStartSimulation = async () => {
  ...
  // Primero reiniciar todos los estados para no ser afectados por la simulación anterior
  resetAllState()
  
  isStarting.value = true
  startError.value = null
  addLog('Iniciando simulación paralela en dos plataformas...')
  ...
  try {
    const params = {
      ...
      enable_graph_memory_update: true  // Activar actualización dinámica del grafo
    }
    
    if (props.maxRounds) {
      params.max_rounds = props.maxRounds
      addLog(`Configurando máximo de rondas de simulación: ${props.maxRounds}`)
    }
    
    addLog('Modo de actualización dinámica del grafo activado')
    
    const res = await startSimulation(params)
    
    if (res.success && res.data) {
      if (res.data.force_restarted) {
        addLog('✓ Logs de simulación anteriores limpiados, reiniciando simulación')
      }
      addLog('✓ Motor de simulación iniciado correctamente')
      ...
      startError.value = res.error || 'Error al iniciar'
      addLog(`✗ Error al iniciar: ${res.error || 'Error desconocido'}`)
...
const handleStopSimulation = async () => {
  ...
  try {
    ...
    if (res.success) {
      ...
      addLog(`Error al detener: ${res.error || 'Error desconocido'}`)
    }
  } catch (err) {
    addLog(`Excepción al detener: ${err.message}`)
  } finally {
    isStopping.value = false
  }
}
...
// Rastrear la última ronda de cada plataforma para detectar cambios y emitir logs
...
const fetchRunStatus = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // Detectar cambios de ronda por plataforma y emitir logs
      ...
      // Detectar si la simulación ha completado (por runner_status o estado de plataforma)
      const isCompleted = data.runner_status === 'completed' || data.runner_status === 'stopped'
      
      // Verificación extra: si el backend aún no actualizó runner_status pero la plataforma ya reportó completado
      // Determinar por estado de twitter_completed y reddit_completed
      const platformsCompleted = checkPlatformsCompleted(data)
      
      if (isCompleted || platformsCompleted) {
        if (platformsCompleted && !isCompleted) {
          addLog('✓ Simulación de todas las plataformas finalizada detectada')
        }
    ...
    console.warn('Error al obtener estado de ejecución:', err)
  }
}

// Verificar si todas las plataformas habilitadas han completado
const checkPlatformsCompleted = (data) => {
  // Si no hay datos de ninguna plataforma, retornar false
  if (!data) return false
  
  // Verificar el estado de completado de cada plataforma
  const twitterCompleted = data.twitter_completed === true
  const redditCompleted = data.reddit_completed === true
  
  // Si al menos una plataforma completó, verificar si todas las habilitadas completaron
  // Determinar si la plataforma está habilitada por actions_count (si count > 0 o running fue true)
  const twitterEnabled = (data.twitter_actions_count > 0) || data.twitter_running || twitterCompleted
  const redditEnabled = (data.reddit_actions_count > 0) || data.reddit_running || redditCompleted
  
  // Si ninguna plataforma está habilitada, retornar false
...
const fetchRunStatusDetail = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // Agregar nuevas acciones incrementalmente (deduplicadas)
      ...
      // No auto-scroll, dejar que el usuario vea libremente la línea de tiempo
      // Las nuevas acciones se añaden al final
    }
  } catch (err) {
...
const handleNextStep = async () => {
  ...
  if (isGeneratingReport.value) {
    addLog('Solicitud de generación de informe enviada, por favor espera...')
    return
  }
  
  isGeneratingReport.value = true
  addLog('Iniciando generación de informe...')
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      addLog(`✓ Tarea de generación de informe iniciada: ${reportId}`)
      
      // Redirigir a la página del informe
      router.push({ name: 'Report', params: { reportId } })
    } else {
      addLog(`✗ Error al iniciar generación de informe: ${res.error || 'Error desconocido'}`)
      isGeneratingReport.value = false
    }
  } catch (err) {
    addLog(`✗ Excepción al iniciar generación de informe: ${err.message}`)
    isGeneratingReport.value = false
  }
}
...
onMounted(() => {
  addLog('Step3 Ejecución de simulación inicializada')
  if (props.simulationId) {
    doStartSimulation()
  }
})
